...
df0 <- read.csv("./Output_3c/Module1_Review Data with sentiments_PH.csv", stringsAsFactors = F)
df0 <- df0[df0$ReviewLength < 101, ]
dftest <- df0
...
sentimentLevels <- rep(999, m)
for (i in 1:m)
{
  if (df0$ReviewSentiment[i] > 1)
  { 
    sentimentLevels[i] <- 1
  }
  if (df0$ReviewSentiment[i] < -1)
